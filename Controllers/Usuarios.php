...
class Usuarios extends Controllers
{
	...
	public function Usuarios()
	{
		...
		$data['page_tag'] = "Usuarios - Clínica Dental";
		$data['page_title'] = "USUARIOS - Odontólogo <small>Clínica Dental</small>";
	...
	public function delUsuario()
	{
		if ($_POST) {
			if ($_SESSION['permisosMod']['d']) {
				...
				if ($requestDelete) {
					$arrResponse = array('status' => true, 'msg' => 'Se ha eliminado el Odontólogo');
				} else {
					$arrResponse = array('status' => false, 'msg' => 'Error al eliminar el Odontólogo.');
				}
				echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
			}
		}
	...
	public function getSelectDentista()
	{
		$htmlOptions = '<option value="">SELECCIONE UN DENTISTA</option>'; // Opción inicial por defecto
		...
		if (count($arrData) > 0) {
			foreach ($arrData as $servicio) {
				if ($servicio['status'] == 1) {
					$htmlOptions .= '<option value="' . $servicio['idpersona'] . '">' . $servicio['nombre_completo'] . '</option>';
				}
			}
		} else {
